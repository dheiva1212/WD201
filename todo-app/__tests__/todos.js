...
describe("Todo Application", function () {
  ...
  test("Creates a todo and responds with json at /todos POST endpoint", async () => {
    ...
    const parsarday = JSON.parse(response.text);
    expect(parsarday.id).toBeDefined();
  });
  ...
  test("Marks a todo with the given ID as complete", async () => {
    ...
    const parsarday = JSON.parse(response.text);
    const todoID = parsarday.id;

    expect(parsarday.completed).toBe(false);
  ...
  test("Fetches all todos in the database using /todos endpoint", async () => {
    ...
    expect(parsarday.length).toBe(4);
    expect(parsarday[3]["title"]).toBe("Buy ps3");
  });
  ...
  test("Deletes a todo with the given ID if it exists and sends a boolean response", async () => {
    ...
    const parsarday = JSON.parse(response.text);
    const todoDS = parsarday.id;

    const deleteTodoResponses = await agent.delete(`/todos/${todoDS}`).send();
